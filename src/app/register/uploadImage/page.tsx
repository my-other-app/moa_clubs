...
export default function UploadImage() {
  ...
  useEffect(() => {
    const savedLogo = storage.getSessionItem('clubLogo');
  ...
  const handleLogoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      storage.setSessionItem('clubLogo', previewURL);
    }
  };

  const handleContinue = () => {
    navigateTo('/register/interestedArea');
  };
      ...
      <div className="bg-white p-8 rounded-xl w-96 border-gray-700 border">
        ...
        <div className="justify-center items-center flex">
          <button
            ...
            className={`w-64 h-15 text-2xl bg-[#2C333D] text-white p-2 rounded-lg font-semibold hover:bg-gray-800 mt-4 ${!clubLogoPreview ? "opacity-50 cursor-not-allowed" : ""
              }`}
          >
            CONTINUE
          </button>
